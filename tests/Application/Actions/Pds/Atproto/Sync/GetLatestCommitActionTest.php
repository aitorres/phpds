...
class GetLatestCommitActionTest extends TestCase
{
    ...
    public function testActionReturnsCidAndRevForKnownRepo(): void
    {
        ...
        $action = new GetLatestCommitAction($logger, $settings, $repoProphecy->reveal(), $factory);

        $request = $this->createRequest('GET', '/xrpc/com.atproto.sync.getLatestCommit')
            ->withQueryParams(['did' => 'did:web:alice.pds.test']);
        $response = (new ResponseFactory())->createResponse();

        $actualResponse = $action($request, $response, []);

        $expected = json_encode(['cid' => 'bafyHead', 'rev' => '3kabc'], JSON_PRETTY_PRINT);
    ...
    public function testActionThrowsXrpcInvalidRequestWhenDidParamMissing(): void
    {
        ...
        $action = new GetLatestCommitAction($logger, $settings, $repo->reveal(), $factory);
        ...
        try {
            ...
            $this->assertSame(
                'Invalid com.atproto.sync.getLatestCommit params: Missing required key "did"',
                $e->getMessage()
            );
        }
    }
    ...
    public function testActionThrowsRepoNotFoundWhenActorMissing(): void
    {
        ...
        $action = new GetLatestCommitAction(
            $logger,
            $settings,
            $repoProphecy->reveal(),
            $factory->reveal()
    ...
    public function testActionThrowsRepoNotFoundWhenRepoRootMissing(): void
    {
        $logger = $this->prophesize(LoggerInterface::class)->reveal();
        $settings = new Settings([]);
        ...
        $actor = $this->makeActor('did:web:alice.pds.test');
        $repoProphecy = $this->prophesize(ActorRepository::class);
        $repoProphecy->findActorByDid('did:web:alice.pds.test')->willReturn($actor);

        $factory = $this->makeFactory('did:web:alice.pds.test', null);
    ...
    public function testActionThrowsRepoTakendownWhenActorIsTakendown(): void
    {
        $logger = $this->prophesize(LoggerInterface::class)->reveal();
        $settings = new Settings([]);
        ...
        try {
            ...
            $this->assertSame('RepoTakendown', $e->getError());
            $this->assertSame(400, $e->getStatusCode());
        }
    }

    public function testActionThrowsRepoDeactivatedWhenActorIsDeactivated(): void
    {
        $logger = $this->prophesize(LoggerInterface::class)->reveal();
        $settings = new Settings([]);
        ...
        try {
            ...
            $this->assertSame('RepoDeactivated', $e->getError());
            $this->assertSame(400, $e->getStatusCode());
        }
    }
