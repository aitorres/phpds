...
class SqliteAccountDeviceRepositoryTest extends TestCase
{
    private function newRepo(): SqliteAccountDeviceRepository
    {
        $db = new Database(':memory:');
        AccountSchema::apply($db);

        return new SqliteAccountDeviceRepository($db);
    }
    ...
    public function testFindByDidAndDeviceId(): void
    {
        ...
        $found = $repo->findByDidAndDeviceId('did:plc:alice', 'dev-1');
        $this->assertSame('dev-1', $found->getDeviceId());
    }
    ...
    {
        $repo = $this->newRepo();
        $repo->save($this->makeEntry('did:plc:alice', 'dev-1'));
        $repo->save($this->makeEntry('did:plc:alice', 'dev-2'));
        $repo->save($this->makeEntry('did:plc:bob', 'dev-3'));
